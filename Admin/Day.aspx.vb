...
Partial Class Admin_Day
    Inherits System.Web.UI.Page
    Dim dm As New DataManager
    Protected Sub btnInsert_Click(sender As Object, e As EventArgs) Handles btnInsert.Click
        If (btnInsert.Text = "Insert") Then
            Dim dt As Data.DataTable = dm.getdata("Select * from Day where UPPER(Day) like '" & txtCityName.Text.ToUpper & "'")
            If (dt.Rows.Count > 0) Then
                MsgBox("Day Already Exists", MsgBoxStyle.Exclamation, "Alert")
            Else

                dm.setdata("Insert into Day(Day)values('" & txtCityName.Text & "')")
        ...
        Else
            ...
            If (dt.Rows.Count > 0) Then
                MsgBox("Day Already Exists", MsgBoxStyle.Exclamation, "Alert")
            Else
                dm.setdata("Update Day set Day='" & txtCityName.Text & "' where Day_id=" & GridView1.SelectedValue)
                MultiView1.ActiveViewIndex = 0
                GridView1.DataBind()
            End If
        End If
    ...
    Protected Sub GridView1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GridView1.SelectedIndexChanged
        Dim dt As Data.DataTable = dm.getdata("Select * from Day where Day_id=" & GridView1.SelectedValue)
