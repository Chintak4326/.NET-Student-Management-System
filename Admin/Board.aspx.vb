...
Partial Class Admin_Board
    ...
    Protected Sub btnInsert_Click(sender As Object, e As EventArgs) Handles btnInsert.Click
        If (btnInsert.Text = "Insert") Then
            Dim dt As Data.DataTable = dm.getdata("Select * from Board where UPPER(Board_name) like '" & txtCityName.Text.ToUpper & "'")
            If (dt.Rows.Count > 0) Then
                MsgBox("Board Already Exists", MsgBoxStyle.Exclamation, "Alert")
            Else

                dm.setdata("Insert into Board(Board_name)values('" & txtCityName.Text & "')")
        ...
        Else
            ...
            If (dt.Rows.Count > 0) Then
                MsgBox("Board Already Exists", MsgBoxStyle.Exclamation, "Alert")
            Else
                dm.setdata("Update Board set Board_name='" & txtCityName.Text & "' where Board_id=" & GridView1.SelectedValue)
                MultiView1.ActiveViewIndex = 0
                GridView1.DataBind()
            End If
        End If
    ...
    Protected Sub GridView1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles GridView1.SelectedIndexChanged
        Dim dt As Data.DataTable = dm.getdata("Select * from Board where Board_id=" & GridView1.SelectedValue)
